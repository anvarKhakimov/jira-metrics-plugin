      ...
      <Paper style={{ padding: '10px' }}>
        ...
        {statusDurations.map((status) => (
          <div style={styles.row} key={status.name}>
            ...
            <span style={styles.leadTime}>{durationToReadableFormat(status.duration)}</span>
          </div>
        ))}
...
function ControlChart({ displayedTasks, cfdData, selectedColumns, timeframeFrom, timeframeTo }) {
  const [activeTask, setActiveTask] = useState(null);

  const transformedData = useMemo(() => {
    let earliestDate = new Date(timeframeTo).getTime();

    const filteredTasks = Object.entries(displayedTasks).map(([taskId, taskData]) => {
  ...
  const xLabels = useMemo(
    () => generateIntervalLabels(timeframeFrom, timeframeTo),
    ...
    <ResponsiveContainer width="100%" height={600}>
      ...
      >
        ...
        <Tooltip
          ...
          active={!!activeTask}
        />
        <Scatter name="Tasks" data={transformedData} fill="#8884d8" onClick={handleScatterClick} />
      </ScatterChart>
    </ResponsiveContainer>
  );
}
